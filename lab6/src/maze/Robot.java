...
public class Robot {
    ...
    /**
     * Implements a stateful maze solver using the right hand rule, with no loop prevention.
     */
    public void escape() {
        if (atExit()) return;

        // High priority: move to the right if possible
        turnRight();
        boolean wallOnRight = canMove();
        if (wallOnRight) {
            move();
            escape();
            return;
        ...
        // Low priority: turn left, i.e. handle the case of left bends (not an intersection)
        turnLeft();
        if (canMove()) {
            move();
            escape();
        ...
        // U-turn, i.e. a dead-end so we backtrack
        turnLeft();
        move();
        escape();
    }
